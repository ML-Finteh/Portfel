...
def resolve_csv_file(prefix: str, data_dir: str) -> Optional[str]:
    """
    Находит CSV или TXT файл в data_dir, имя которого начинается с prefix.
    Возвращает полный путь или None.
    """
    if not os.path.isdir(data_dir):
        return None
    for fname in os.listdir(data_dir):
        if fname.lower().endswith(('.csv', '.txt')) and fname.startswith(prefix):
...
CSV_MAP = {
    ...
    'RATE_PERIODS': {
        'prefix': 'RATE_PERIODS',      # найдёт RATE_PERIODS_2026.07.03.txt и т.д.
        'date_col': 'Date',
        'value_col': 'Rate',
        'date_format': '%d.%m.%Y',
    },
...
MOEX_API_ENDPOINTS = {
    ...
    'GOLD': {
        'url_template': 'https://iss.moex.com/iss/history/engines/currency/markets/selt/boards/CETS/securities/{ticker}.csv',
        'ticker': 'GLDRUB_TOM', 'sep': ';', 'date_col': 'TRADEDATE', 'value_col': 'CLOSE',
        'high_col': 'HIGH', 'low_col': 'LOW'
    },
    'USD_RUB': {
        'url_template': 'https://iss.moex.com/iss/history/engines/currency/markets/selt/boards/CETS/securities/{ticker}.csv',
...
RATE_PERIODS = []  # loaded from CSV file
...
async def async_fetch_moex_history(
    ...
    while page < max_pages:
        ...
        try:
            # MOEX отдаёт в cp1251 с секциями history и history.cursor
            text = content.decode('cp1251', errors='ignore')

            # Извлекаем только секцию history (между 'history' и 'history.cursor')
            lines = text.split('\n')
            in_history = False
            history_lines = []
            for line in lines:
            ...
            if not history_lines:
                errors.append(f"{asset_name}: пустая секция history в ответе MOEX")
                break

            history_text = '\n'.join(history_lines)
            df = pd.read_csv(StringIO(history_text), sep=cfg['sep'])
        except Exception as e:
        ...
        # Проверяем колонки — MOEX валюты могут не иметь заголовков, попробуем автоопределение
        if cfg['date_col'] not in df.columns or cfg['value_col'] not in df.columns:
            # Для валютных пар MOEX не всегда возвращает заголовки
            if len(df.columns) >= 8 and asset_name in ('USD_RUB', 'CNY_RUB', 'GOLD'):
                # Стандартный формат: BOARDID;TRADEDATE;SECID;SHORTNAME;OPEN;LOW;HIGH;CLOSE;...
                df.columns = ['BOARDID', 'TRADEDATE', 'SECID', 'SHORTNAME', 'OPEN', 'LOW', 'HIGH', 'CLOSE'] + [f'COL_{i}' for i in range(len(df.columns)-8)]
            else:
...
async def async_load_key_rate_hybrid(
    ...
    """Загружает ключевую ставку: CSV файл + догрузка с ЦБ РФ."""
    metadata = {
        'source': None,
        'errors': [],
        'warnings': [],
    ...
    # 1. Загрузка из CSV файла по префиксу
    csv_series = pd.Series(dtype=float)
    csv_path = resolve_csv_file('RATE_PERIODS', config.data_dir)

    if csv_path:
        try:
            df = pd.read_csv(csv_path)
            date_col = CSV_MAP['RATE_PERIODS']['date_col']
            val_col = CSV_MAP['RATE_PERIODS']['value_col']
            date_fmt = CSV_MAP['RATE_PERIODS'].get('date_format', '%d.%m.%Y')

            dates = pd.to_datetime(df[date_col], format=date_fmt, errors='coerce')
            rates = pd.to_numeric(df[val_col], errors='coerce')

            valid = dates.notna() & rates.notna()
            df_valid = df[valid].sort_values(date_col)

            # Создаём Series из дат и ставок, сортируем по дате
            rate_df = pd.DataFrame({'date': dates[valid], 'rate': rates[valid]}).sort_values('date')
            rate_df = rate_df.drop_duplicates(subset='date', keep='last')

            # Создаём полный диапазон рабочих дней от первой даты до сегодня
            full_dates = pd.bdate_range(rate_df['date'].min(), pd.Timestamp.now(), freq='B')

            # Расширяем DataFrame на все даты и заполняем forward fill
            full_df = pd.DataFrame({'date': full_dates})
            full_df = full_df.merge(rate_df, on='date', how='left')
            full_df['rate'] = full_df['rate'].ffill()

            csv_series = pd.Series(data=full_df['rate'].values, index=full_df['date'], name='key_rate')
            csv_series = csv_series.dropna()
            metadata['source'] = 'CSV'
            metadata['last_date'] = csv_series.index.max()
            logger.info(f"  Ключевая ставка (CSV): {len(df_valid)} записей, до {csv_series.index.max().date()} ({csv_series.iloc[-1]:.2f}%)")
        except Exception as e:
            metadata['errors'].append(f"[CSV] Ошибка загрузки RATE_PERIODS: {e}")
    else:
        metadata['errors'].append("[CSV] Файл RATE_PERIODS не найден в data_dir")

    if csv_series.empty:
        metadata['errors'].append("Не удалось загрузить ключевую ставку из CSV")
        # Fallback: пытаемся загрузить только с ЦБ РФ
        logger.info("  → Попытка загрузки ключевой ставки только с ЦБ РФ...")
        ...
        if not web_series.empty:
            ...
            metadata['last_date'] = web_series.index.max()
            return web_series, metadata
        return pd.Series(dtype=float), metadata

    # 2. Проверка актуальности и догрузка с ЦБ РФ
    last_csv_date = csv_series.index.max()
    days_old = (pd.Timestamp.now().normalize() - last_csv_date).days
    threshold = config.freshness_thresholds.get('key_rate', 7)

    if days_old <= threshold or config.offline_mode:
        if config.offline_mode:
            logger.info(f"  Ключевая ставка (offline): до {last_csv_date.date()}")
        else:
            logger.info(f"  Ключевая ставка актуальна (задержка {days_old} дней)")
        return csv_series, metadata

    metadata['warnings'].append(f"CSV устарел на {days_old} дней — попытка догрузки с ЦБ РФ")
    ...
    if not web_series.empty:
        new_records = web_series[web_series.index > last_csv_date]
        if not new_records.empty:
            combined = pd.concat([csv_series, new_records]).sort_index()
    ...
    metadata['warnings'].append("Догрузка с ЦБ РФ не удалась — используется CSV")
    return csv_series, metadata
...
def simulate_portfolio(
    ...
    for name, price in prices.items():
        returns[name] = price.pct_change().reindex(all_dates, fill_value=0.0).fillna(0.0)

    # Сигналы на каждую дату
    sig_aligned = {}
    for name, sig in signals.items():
        sig_aligned[name] = sig.reindex(all_dates, fill_value=0.0).fillna(0.0)
    ...
    for date in all_dates:
        ...
        day_signals = {}
        for name in prices:
            ret = returns[name].loc[date] if date in returns[name].index else 0.0
            day_returns[name] = 0.0 if pd.isna(ret) else float(ret)
...
def generate_report(
    ...
    for name, meta in data_status.items():
        ...
        lines.append(f"  {name:12s} | источник: {str(status):12s} | последняя дата: {last_str}")
